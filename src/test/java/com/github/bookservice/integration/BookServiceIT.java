...
public class BookServiceIT {
	...
	public void modifyBook() throws Exception {
		...
		ResponseEntity<Book> response = restTemplate.exchange(BASE_URL, HttpMethod.PUT, new HttpEntity<Book>(book), Book.class);
		Book persistedBook = response.getBody();
	...
	public void removeBook() throws Exception {
		ResponseEntity<Object> response = restTemplate.exchange(BASE_URL+BOOK_ID, HttpMethod.DELETE, null, Object.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);
	}
	
	@Test
	public void removeBook_throwsBookNotFoundExcpetion() throws Exception {
		ResponseEntity<String> response = restTemplate.exchange(BASE_URL+BOOK_ID, HttpMethod.DELETE, null, String.class);
		assertBookNotFoundScenario(response);
	}
	
	@Test
	public void getBook_throwsBookNotFoundExcpetion() throws Exception {
		ResponseEntity<String> response = restTemplate.exchange(BASE_URL+BOOK_ID, HttpMethod.GET, null, String.class);
		assertBookNotFoundScenario(response);
	}
	...
	public void modifyBook_throwsBookNotFoundExcpetion() throws Exception {
		Book book = new Book();
		book.setId(1);
		ResponseEntity<String> response = restTemplate.exchange(BASE_URL, HttpMethod.PUT, new HttpEntity<Book>(book), String.class);
		assertBookNotFoundScenario(response);
	}
